...
library provides the `define_gv!` to generate the appropriate struct types.

If we have a type with the same representation as the underlying byes we can
just cast the data to the appropriate type and then read it.
...
| **o**         | `gvariant::Str`                                                                         | No                              |
| **g**         | `gvariant::Str`                                                                         | No                              |
| **v**         | `gvariant::Variant`                                                                     | No                              |
| **m**s        | `gvariant::NonFixedSizeMaybe<gvariant::Str>` - and similarly for all non-`Sized` types  | No                              |
| **m**i        | `gvariant::FixedSizeMaybe<i32>` - and similarly for all `Sized` types                   | No                              |
| **a**s        | `gvariant::NonFixedWidthArray<gvariant::Str>` - and similarly for all non-`Sized` types | No                              |
| **a**i        | `[i32]` and similarly for all Sized types                                               | No                              |
| **(**sv**)**  | Custom struct generated by `gv_define!("(sv)")`. Implements `.to_tuple()` method        | Yes if all children are `Sized` |
| **{**si**}**  | Custom struct generated by `gv_define!("{si}")` Macro. Implements `.to_tuple()` method  | Yes if all children are `Sized` |
