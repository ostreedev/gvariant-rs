...
use gvariant::{Cast, SerializeTo, Structure, decl_gv, gv};
...
decl_gv!{
    ...
    type SISI = "(sisi)";
    type SUTUTY = "(sututy)";
    type Sututysis = "(sututysis)";
    type Ayayayayay = "(ayayayayay)";
    type ysas = "((ys)as)";
    type Ayy = "(ayy)";
    type GVCommit = "(a{sv}aya(say)sstayay)";
    type GVTreeMeta = "(a(say)a(sayay))";
}

#[test]
fn test_struct_into_tuple() {
    let t = <gv!("()") as Cast>::from_aligned_slice(empty_aligned()).to_tuple();
    assert_eq!(t, ());

    let t = <gv!("(i)")>::from_aligned_slice(empty_aligned()).to_tuple();
    assert_eq!(t, (&0,));

    let t = <gv!("(s)")>::from_aligned_slice(empty_aligned()).to_tuple();
    assert_eq!(t.0.to_str(), "");

    let buf = copy_to_align(b"\x06\x00\x00\x00\x03\x00\x00\x00");
    let t: (&i32, &i32) = <gv!("(ii)")>::from_aligned_slice(buf.as_ref()).into();
    assert_eq!(t, (&6, &3));

    let buf = copy_to_align(b"\x06\x00\x00\x00super\x00");
    let t: (&i32, &gvariant::Str) = <gv!("(is)")>::from_aligned_slice(buf.as_ref()).to_tuple();
    assert_eq!(t.0, &6);
    assert_eq!(t.1, "super");
}

fn check_serialize<M: Cast + ?Sized, T: SerializeTo<M>>(x: T, expected: &[u8]) {
    let mut v = vec![];
...
fn test_struct_serialisation() {
    // () - Unit type
    check_serialize::<gv!("()"), _>(&(), b"\0");

    // (i) - Fixed size
    check_serialize::<gv!("(i)"), _>(&(87654321,), b"\xb1\x7f\x39\x05");

    // (iy) - Fixed size with padding at end
    check_serialize::<gv!("(iy)"), _>(&(87654321, 6), b"\xb1\x7f\x39\x05\x06\x00\x00\x00");

    // (yi) - Fixed size with padding in middle
    check_serialize::<gv!("(yi)"), _>(&(6, 87654321), b"\x06\x00\x00\x00\xb1\x7f\x39\x05");

    // (yyyyi) - Fixed size no padding
    check_serialize::<gv!("(yyyyi)"), _>(&(6, 5, 4, 3, 87654321), b"\x06\x05\x04\x03\xb1\x7f\x39\x05");

    // (s) - Non fixed size
    check_serialize::<gv!("(s)"), _>(&("Hello!",), b"Hello!\0");

    // (si) - Non fixed size ends with fixed aligned value
    check_serialize::<gv!("(si)"), _>(&("Hello!!", 87654321), b"Hello!!\0\xb1\x7f\x39\x05\x08");
    check_serialize::<gv!("(si)"), _>(&("Hello!", 87654321), b"Hello!\0\0\xb1\x7f\x39\x05\x07");
    check_serialize::<gv!("(si)"), _>(&("Hello", 87654321), b"Hello\0\0\0\xb1\x7f\x39\x05\x06");
    check_serialize::<gv!("(si)"), _>(&("Hell", 87654321), b"Hell\0\0\0\0\xb1\x7f\x39\x05\x05");

    // (is) - Non fixed size starts with fixed aligned value
    check_serialize::<gv!("(is)"), _>(&(87654321, "Hello"), b"\xb1\x7f\x39\x05Hello\0");

    // (isis) - 2 non-fixed size elements, 1 framing offset
    check_serialize::<gv!("(isis)"), _>(&(87654321, "Hello", 0x12345678, "Byee"), b"\xb1\x7f\x39\x05Hello\0\0\0\x78\x56\x34\x12Byee\0\x0a");

    // (sisi) - 2 non-fixed size elements, 2 framing offsets
    check_serialize::<gv!("(sisi)"), _>(&("Hello", 87654321, "Byee", 0x12345678), b"Hello\0\0\0\xb1\x7f\x39\x05Byee\0\0\0\0\x78\x56\x34\x12\x11\x06"
    );
    ...
    check_serialize::<gv!("(sututy)"), _>(&input, expected);
    ...
    check_serialize::<gv!("(sututysis)"), _>(&input, expected);
}
...
fn test_complex_types() {
    ...
    let (files, dirs) = <gv!("(a(say)a(sayay))")>::from_aligned_slice(buf.as_ref()).into();
    ...
    check_serialize::<gv!("(a(say)a(sayay))"), _>(&expected, buf.as_ref());

    let buf = copy_to_align(include_bytes!(
        "0bf6200211dd4fd63be6e9bc5c90bea645e2696c0117b05f83562081813a5b94.commit"
    ));
    let commit = <gv!("(a{sv}aya(say)sstayay)")>::from_aligned_slice(buf.as_ref());
    let (
        metadata,
        _parent_checksum,
        _related_objects,
    ...
    let t = <gv!("(sututysis)")>::from_aligned_slice(buf.as_ref()).to_tuple();
    assert_eq!(t.0, "");
...
fn test_spec_examples() {
    ...
    let (s, i) = <gv!("(si)")>::from_aligned_slice(data.as_ref()).to_tuple();
    assert_eq!(s, "foo");
    assert_eq!(*i, -1);
    check_serialize::<gv!("(si)"), _>(&("foo", -1), &**data.as_ref());
    ...
    let ns = <gv!("((ys)as)")>::from_aligned_slice(b"ican\0has\0strings?\0\x04\x0d\x05".as_aligned());
...
fn test_non_normal_values() {
    ...
    assert_eq!(
        <gv!("ab")>
            ::from_aligned_slice([0x01u8, 0x00, 0x03, 0x04, 0x00, 0x01, 0xff, 0x80, 0x00].as_aligned())
            .iter()
            .map(|x| x.to_bool())
            .collect::<Vec<_>>(),
    ...
    assert_eq!(<gv!("s")>::from_aligned_slice(b"foo\0bar\0".as_aligned()), "");

    // String with embedded nul but none at end
    assert_eq!(<gv!("s")>::from_aligned_slice(b"foo\0bar".as_aligned()), "");

    // Wrong size for fixed-size maybe
    assert!(<gv!("mi")>::
        from_aligned_slice(&*copy_to_align(&[0x33u8, 0x44, 0x55, 0x66, 0x77, 0x88]))
        .to_option()
        .is_none());
    ...
    let gv_as = <gv!("as")>::from_aligned_slice(b"foo\0bar\0baz\0\x04\x00\x04\x0c".as_aligned());
    assert_eq!(
        gv_as.into_iter().map(|x| x.to_str()).collect::<Vec<_>>(),
        ["foo", "", "foo", ""]
    );
    ...
    let t = <gv!("(ayayayayay)")>::from_aligned_slice([0x03u8, 0x02, 0x01].as_ref().as_aligned())
        .to_tuple();
    assert_eq!(
        t,
        (
